...
| **G36** | **Identity collision on the way back** (found 2026-09-14). The node mints ids from its own `AUTO_INCREMENT`; so does the state for other societies in the same table; a ledger pack's `REPLACE INTO` can land on another society's row. 549 of 839 society tables have a single-column AI PK (481 BIGINT, 68 INT); 4 include PacsId. `IdGenerationOptions` exists, 0 implementations. | **OPEN.** Recommended: a per-node id range set at carve-in (`MySqlBootstrapper` after `.epdata`), 68 INT→BIGINT in the baseline, the state's ingest refuses out-of-range rows, an ordinal registry per PacsId. Blocks the state-side ingest. | — |
| **G35** | **Control files were read loose from the stick.** The manifest hashed and signed payloads; the pipeline read `config/service-map.yaml`, `config-templates/` and `db/stable_baseline_ddl.sql` straight from the medium directory — none a payload. An attacker who could not re-sign the manifest could still swap the schema or the map. | **Closed 2026-09-13:** `VerifiedMedia` — three control payloads by name (`config`, `config-templates`, `db`), re-hashed at the moment of use, extracted root-only; a loose file is ignored; a missing control payload refuses by name. 5 tests + a pipeline test. | — |
...
- [~] 8. Implement Installer.Actions — Fresh Install
  ...
  - [~] 8.10 Integration tests for fresh install — **2026-09-13:** `tests/integration/debian-e2e.sh` + `.github/workflows/e2e-debian.yml` (nightly, on demand): Debian 12 with systemd as PID 1, the installer self-contained, a real MySQL 8.4, the harness as the application payload; install → health → backup → restore → repair → uninstall with a COUNT at every gate. **Not yet run** - it needs the privileged container the workflow provides; the first run is the next thing to watch. `PackLiveTests` runs the pack round trip against a real MySQL with real mysqldump/mysql on the CI Linux leg. *Was:* **`Installer.IntegrationTests/UnitTest1.cs` is one placeholder fact.**
...
- [~] 12. Implement Installer.CLI (silent mode)
  ...
  - [x] 12.4 File logging — **Done 2026-09-13:** `SharedKernel/Hosting/RunLogFileProvider`, one file per run under `<DataRoot>/logs/installer/`, flushed on every line, root-only; console unless `--quiet`, the file always, and said aloud when it cannot be opened.
  - [~] 12.5 Integration tests for silent install — the Debian end-to-end (8.10) runs `--apply` unattended; `--quiet` is asserted by the CLI tests. Needs the first e2e run.
...
- [x] **X4. A guard against this file drifting again.** *Done 2026-09-13:* `build/check-tasks-evidence.py` fails when an `[x]` item names a code path that does not exist under src/ or tests/; it caught a moved file on its first run. Runs in ci.yml. Originally: A check that fails when an item is
